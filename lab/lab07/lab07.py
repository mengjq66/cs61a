...
def is_bst(t):
    ...
    def b_min(tree):
        if tree.is_leaf():
            return tree.label
        return min([tree.label] + [b_min(b) for b in tree.branches])

    def b_max(tree):
        if tree.is_leaf():
            return tree.label
        return max([tree.label] + [b_max(b) for b in tree.branches])
    ...
    if t.is_leaf():
        ...
        return is_bst(t.branches[0]) and is_bst(t.branches[1]) and b_max(t.branches[0]) <= t.label and b_min(t.branches[1]) > t.label
