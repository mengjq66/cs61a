...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        # Evaluate operator (recursive call to scheme_eval)
        procedure = scheme_eval(first, env)
        check_procedure(procedure)
        # Without evaluating all of the operands, check if procedure is MacroProcedure
        # if so, evaluate the expression on the unevaluated operands passed in
        if isinstance(procedure, MacroProcedure):
            return scheme_eval(procedure.apply_macro(rest, env), env)

        # Evaluate all of the operands
        opers = rest.map(lambda arg: scheme_eval(arg, env))
        # Apply procedure on evaluated operands
        return scheme_apply(procedure, opers, env)
        # END PROBLEM 4
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Incorrect number of arguments passed in")
        # END PROBLEM 3
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        # only one expression left in expressions, tail recursive
        return scheme_eval(expressions.first, env, True)
    else:
        # if the first expression evaluates to false, short circuit to false
        if scheme_falsep(scheme_eval(expressions.first, env)):
            return False
        # evaluates to true so recurse with expressions.rest
...
def do_or_form(expressions, env):
    if expressions == nil:
        ...
        # if the first expression evaluates to false, continue
        if scheme_falsep(scheme_eval(expressions.first, env)):
            return do_or_form(expressions.rest, env)
        # evaluates to true so return evaluated operand
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        # check if expression has length 2 and only 2
        check_form(bindings.first, 2, 2)

        # Update formals and values
        formals = Pair(bindings.first.first, formals)
        values = Pair(scheme_eval(bindings.first.rest.first, env), values)

        bindings = bindings.rest

    # Raise SchemeError if formals is not list of symbols or if any symbol is repeated
    check_formals(formals)
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 20
    # check if it is a pair with two expressions and two expressions only (min length 2, max length 2)
    check_form(expressions, 2, 2)
    target = expressions.first
    # check if the first expression of the pair is a pair with min length 1
    check_form(target, 1)

    # create a MacroProcedure and bind it to the given name as in do_define_form
    if scheme_symbolp(target.first):
        macro_procedure = MacroProcedure(target.rest, expressions.rest, env)
